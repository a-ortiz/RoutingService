...
public final class TourController {
    ...
    private final static class CreateTourPayload {
        private @Getter @Setter List<CZCoordinate> coordinates;

        @JsonIgnore
        public boolean isValid() {
            if (coordinates.size() <= 1) return false;
            for (final CZCoordinate coordinate : coordinates) if (!coordinate.isValid()) return false;
            return true;
        }

        @JsonCreator
        CreateTourPayload(@JsonProperty("points") List<CZCoordinate> coordinates) {
